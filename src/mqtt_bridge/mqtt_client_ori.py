...
def default_mqtt_client_factory(params):
    ...
    if client_params.pop("client_id")!=0:
       	client = mqtt.Client(**client_params)
    else:
	client_params=client_params.remove("client_id")
    	client = mqtt.Client(**client_params)
    ...
    account_params = params.get('user_setting', {})
    
    usr_params = account_params.pop("user")
    pwd_params = account_params.pop("password")

    if usr_params!=0 and pwd_params!=0:
    	client.username_pw_set(usr_params,pwd_params)
    else:
        account_params=account_params.remove("user")
        account_params=account_params.remove("password")
